...
library InterestLib {
    function calculateInterestMatured(
        ...
        for (uint256 i = 0; i < Lender[user].length; i++) {
            ...
            totalInterest +=
                ((block.timestamp - Lender[user][i].depositTime) *
                    lendAPR *
                    Lender[user][i].amount) /
                (100 * 365 days);
        }
    ...
    function calculatePenalty(
        ...
        uint256 daysLate = (block.timestamp -
            Borrower[borrowerAddress][borrowNumber].dueDate) / 1 days;
        uint256 penaltyAmount = (Borrower[borrowerAddress][borrowNumber]
            .borrowAmount *
            penalty *
            daysLate) / (100 * 365);
...
contract LoanManager {
    ...
    mapping(address => uint256) public borrowCount; //total numbers of borrow position for every user
    ...
    function checkCategoryRegistration(
        address[] storage list,
        address[] storage list2
    ) internal {
        bool found;
        bool found2;
    ...
    function updateInterestMatured() public returns (uint256) {
        uint256 totalInterest = Lender.calculateInterestMatured(
            msg.sender,
            lendAPR
        );
    ...
    function withdraw(
        uint256 amount
    ) public checkNetPosition(amount) nonReentrant {
        require(amount > 0, "Amount must be greater than zero");
        require(
            netPosition[msg.sender] >= amount,
            "Not enough funds in deposits to cover withdrawal"
        );
        ...
        nonReentrant
    {
        ...
        uint256 _paybackAmount = amount +
            ((amount * APR * _weeks) / (100 * 52));
    ...
    function calculatePenalty(
        ...
        uint256 penaltyAmount = Borrower.calculatePenalty(
            borrowNumber,
            penalty,
            msg.sender
        );
    ...
    function payBorrow(uint256 borrowNumber) public payable nonReentrant {
        ...
        if (msg.value > Borrower[msg.sender][borrowNumber].paybackAmount) {
            ...
            Borrower[msg.sender][borrowNumber].state = borrowState.Paid;
            (bool success, ) = msg.sender.call{value: difference}("");
            require(success, "Transaction failed");

            // Case 2: Exact payment
        } else if (
            msg.value == Borrower[msg.sender][borrowNumber].paybackAmount
        ) {
            netPosition[msg.sender] += msg.value;
            Borrower[msg.sender][borrowNumber].state = borrowState.Paid;
